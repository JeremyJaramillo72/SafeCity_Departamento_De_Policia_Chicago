...
print("[INFO] Iniciando correccion y migracion con redistribucion de años...")
t0 = time.time()
...
# 1. Eliminar los registros de la ultima migracion
print("[DELETE] Eliminando los 300,000 registros anteriores (> 500,007)...")
t_del = time.time()
client.execute("ALTER TABLE default.chicago_crimes DELETE WHERE id > 500007")
client.execute("ALTER TABLE default.incidente_delito DELETE WHERE id_incidente_delito > 500006")
...
# 2. Conectar a SQLite de PocketBase directamente en el volumen local
...
# -------------------------------------------------------------------
# 3. Transformacion de datos con Aleatoriedad de Años
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 4. Inyeccion en ClickHouse Local
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 5. Ejecutar el script SQL de Migracion de Datos
# -------------------------------------------------------------------
print("\n[MIGRATE] Iniciando ejecucion de scripts de migracion...")

# (Omitimos catalogo_ubicacion, codigo_penal y estacion_policial porque ya se insertaron los unicos antes
#  aunque podemos volver a correrlos con seguridad por el condicional NOT IN)
...
# -------------------------------------------------------------------
# 6. Verificaciones Finales de Totales
# -------------------------------------------------------------------
