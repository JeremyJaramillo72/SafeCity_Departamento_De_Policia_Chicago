...
print("[INFO] Iniciando extraccion de 300k registros (OFFSET 900,000)...")
t0 = time.time()
...
client = Client(host='localhost', port=9000, user='default', password='password12345', secure=False)
...
SELECT coalesce((SELECT max(id_ubicacion) FROM catalogo_ubicacion), 0) + rowNumberInAllBlocks() + 1 AS id_ubicacion, location_description, false
FROM (SELECT DISTINCT location_description FROM dataset_crudo WHERE location_description != '' AND location_description NOT IN (SELECT descripcion_lugar FROM catalogo_ubicacion))
""")

print("[MIGRATE] 1.2 Migrando codigo_penal...")
client.execute("""
INSERT INTO codigo_penal (id_codigo, codigo_iucr, codigo_fbi, gravedad_delito)
SELECT coalesce((SELECT max(id_codigo) FROM codigo_penal), 0) + rowNumberInAllBlocks() + 1 AS id_codigo, iucr, fbi_code, primary_type
FROM (SELECT DISTINCT iucr, fbi_code, primary_type FROM dataset_crudo WHERE iucr != '' AND iucr NOT IN (SELECT codigo_iucr FROM codigo_penal) LIMIT 1 BY iucr)
""")

print("[MIGRATE] 1.3 Migrando estacion_policial...")
client.execute("""
INSERT INTO estacion_policial (id_estacion, numero_distrito)
SELECT coalesce((SELECT max(id_estacion) FROM estacion_policial), 0) + rowNumberInAllBlocks() + 1 AS id_estacion, district
FROM (SELECT DISTINCT district FROM dataset_crudo WHERE district != '' AND district NOT IN (SELECT numero_distrito FROM estacion_policial))
""")
...
SELECT 
    ...
    case_number, parseDateTimeBestEffortOrZero(date) AS parsed_date, block, description,
    ifNull(cu.id_ubicacion, 0) AS id_ubicacion, if(lower(arrest) = 'true', true, false) AS parsed_arrest,
    if(lower(domestic) = 'true', true, false) AS parsed_domestic, beat, ifNull(ep.id_estacion, 0) AS id_estacion,
    ward, community_area, toFloat64OrZero(replaceAll(x_coordinate, ',', '.')) AS parsed_x,
    toFloat64OrZero(replaceAll(y_coordinate, ',', '.')) AS parsed_y, toInt32OrZero(year) AS parsed_year,
    parseDateTimeBestEffortOrZero(updated_on) AS parsed_updated, toFloat64OrZero(replaceAll(latitude, ',', '.')) AS parsed_lat,
    toFloat64OrZero(replaceAll(longitude, ',', '.')) AS parsed_lng
FROM (SELECT * FROM dataset_crudo WHERE case_number != '' LIMIT 1 BY case_number) AS dc
...
SELECT coalesce((SELECT max(id_incidente_delito) FROM incidente_delito), 0) + rowNumberInAllBlocks() + 1 AS id_incidente_delito, case_number, iucr, true AS es_delito_primario
FROM (SELECT case_number, iucr FROM dataset_crudo WHERE case_number != '' AND iucr != '' LIMIT 1 BY case_number)
""")
